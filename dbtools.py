...
class Dao(object):
    ...
    def find_all_in_selected_order(self, order):
        c = self._conn.cursor()
        c.execute('SELECT * FROM {} ORDER BY {}'.format(self._table_name , order))
        return c
    ...
    def update(self, set_values, cond):
        ...
        stmt = 'UPDATE {} SET {} WHERE ({})'.format(self._table_name,','.join([set + '=?' for set in set_column_names]), 
